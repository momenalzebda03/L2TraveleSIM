...
//Google pay fixes 
  ...
  isEsimCompatible: boolean | null = null;

  result:any; 

  async checkEsimBeforePurchase() {
    await this.platform.ready();
    try {
      this.result = await new Promise((resolve, reject) => {
        cordova.plugins.esimcheck.getEsimStatus(
          (res: any) => resolve(res),
          (err: any) => reject(err)
        );
      });

      if (this.result?.isEsimSupported) {
        this.isEsimCompatible = true;
      } else {
        this.isEsimCompatible = false;
      }
...
async validate(): Promise<boolean> {
   ...
   // ❌ eSIM NOT compatible → show modal
  if (this.isEsimCompatible === false) {
    const proceed = await this.showEsimWarningModal(); // ✅ wait for user
    return proceed;
  }
 ...
 // ✅ First check eSIM compatibility
  if (!(await this.validate())) {
    return;
  }

  // ✅ Then validate currency/payment rules
  if (!this.validatePaymentCurrency()) {
    return;
  }
...
async showEsimWarningModal(): Promise<boolean> {
  const modal = await this.modalController.create({
    component: ConfirmContinuePage
  });
  await modal.present();
  const { data } = await modal.onDidDismiss();
  return data === true; // ✅ true = continue, false = cancel
}
