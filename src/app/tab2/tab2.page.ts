...
@ViewChild('countrySwiper', { static: false }) countrySwiper: any;
@ViewChild('regionSwiper', { static: false }) regionSwiper: any;
  ...
  constructor(private cdr: ChangeDetectorRef,private translate: TranslateService, private toastController: ToastController, private platform: Platform, private loadingScreen: LoadingScreenAppPage, private http: HttpClient, private navController: NavController, private modalController: ModalController, private apiService: ServicesService, private Router: Router, private elementRef: ElementRef) {
  }
  ...
  ngOnInit() {
    ...
    this.langDefault = window.localStorage.getItem('L2TraveleSIM_language');
     this.isArabic = this.langDefault === 'ar';

  // Set global direction (important for Android)
...
fixSwiperProperly() {
  requestAnimationFrame(() => {
    ...
    const swiperEl =
      this.selectedSegment === 'singlecountry'
        ? this.countrySwiper?.nativeElement
        : this.regionSwiper?.nativeElement;

    if (swiperEl?.swiper) {
      const swiper = swiperEl.swiper;

      swiper.changeLanguageDirection(this.isArabic ? 'rtl' : 'ltr');
      swiper.update();
      swiper.slideTo(0, 0);
    }
  });
