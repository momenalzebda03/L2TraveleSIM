...
@Component({
  selector: 'app-processing-bar-apple-pay-topup',
  templateUrl: './processing-bar-apple-pay-topup.page.html',
  styleUrls: ['./processing-bar-apple-pay-topup.page.scss'],
})
export class ProcessingBarApplePayTopupPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.managingAppLogs("From App Step 3: Credit Topup Apple Pay payment QR code generation started:",this.value.amount, "Credit Top-up");
    this.startProgress();
    
    this.service.stripeApplePayTopUpWallet(this.value, this.value1).then((res: any) => {
      if (res.code == 200) {
      ...
      this.managingAppLogs("From App Step 4 Credit Topup:  Apple Pay payment Success:",this.value.amount, "Credit Top-up");
      } else {
       this.managingAppLogs("From App Step 4 Credit Topup: Apple pay payment Error:" + JSON.stringify(res),this.value.amount, "Credit Top-up");
      }
    }).catch(err => {
     this.managingAppLogs("From App Step 4: Apple pay payment Error from API:" + JSON.stringify(err),this.value.amount, "Credit Top-up");
    });
  }
