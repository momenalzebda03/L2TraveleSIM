...
export class YourPlanPage implements OnInit {
  ...
  bundleDatas: any = [];
  statusOrder: any;
  constructor(private modalController: ModalController, private device: Device, private platform: Platform,private translate: TranslateService,private iab: InAppBrowser,private loadingScreen: LoadingScreenAppPage, private renderer: Renderer2, private el: ElementRef, private service: ServicesService, private navController: NavController, private Router: Router) { }
  tempBundles: any = [];
  networkImages: any = [];
  ...
  bundleName:any;
    ...
    // When the view has loaded, call the function
    ionViewDidEnter() {
      this.getDeviceInfo();
    }
    deviceVersion:any;
    // Method to get the device info
    directInstallOpt:any =true;

    getDeviceInfo() {
      this.platform.ready().then(() => {
        const devicePlatform = this.device.platform; // e.g., 'iOS'
        this.deviceVersion = parseFloat(this.device.version); // Convert to number e.g., 17.4
    
        if (devicePlatform === 'iOS') {
          if (this.deviceVersion >= 17.4) {
            this.directInstallOpt = true;
          } else {
            this.directInstallOpt = false;
          }
        } else {
 ...
 if(this.directInstallOpt ==true)
  ...
  {
    this.errorMSGModal(this.translate.instant('VALIDATION_MSG_BUTTON'), this.translate.instant('iOSversionManualMSG'), this.bundleDatas);
  }
} 
 ...
 // Error Modal
  ...
  // Error Modal
  async errorMSGModal(buttonText: any, msg: any,item: any) {
    const modal = await this.modalController.create({
